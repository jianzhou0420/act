...
class JianRLBenchDataConverter:
    ...
    def __init__(self, data_dir=None):
        ...
        self.folders = sorted([d for d in os.listdir(self.data_dir) if os.path.isdir(os.path.join(self.data_dir, d))], key=self._natural_sort_key)
        self.idx = None
        pass
    ...
    def _load_images(self):
        def load_images_from_folder(folder):
            images = []
            for filename in sorted(os.listdir(folder), key=self._natural_sort_key):
                img_path = os.path.join(folder, filename)
                if os.path.isfile(img_path) and img_path.endswith(('.png', '.jpg', '.jpeg')):
                    img = Image.open(img_path).convert('RGB')
                    img_tensor = torch.tensor(np.array(img), dtype=torch.uint8).permute(2, 0, 1)  # Convert to CxHxW
        ...
        self.data_container.left_shoulder_depth = load_images_from_folder(os.path.join(self.current_folder_path, 'left_shoulder_depth'))
        ...
        self.data_container.right_shoulder_depth = load_images_from_folder(os.path.join(self.current_folder_path, 'right_shoulder_depth'))
        ...
        self.data_container.wrist_depth = load_images_from_folder(os.path.join(self.current_folder_path, 'wrist_depth'))
        ...
        self.data_container.front_depth = load_images_from_folder(os.path.join(self.current_folder_path, 'front_depth'))
        ...
        self.data_container.overhead_depth = load_images_from_folder(os.path.join(self.current_folder_path, 'overhead_depth'))
    ...
    def _save_as_hdf5(self):
        save_dir = "/media/jian/data/rlbench_hdf5/train"
        number = self.idx
        save_path = os.path.join(save_dir, f"episode_{number}.h5")
...
if __name__ == "__main__":
    test = JianRLBenchDataConverter("/media/jian/data/rlbench/train/close_jar/all_variations/episodes/")
